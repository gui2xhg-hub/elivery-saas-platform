          ...
          <section className="lg:col-span-1 bg-gray-900 p-5 rounded-2xl border border-gray-800 space-y-4">
            ...
            {pdvOrderType === 'mesa' && (
              <div className="grid grid-cols-2 gap-2">
                <input type="text" placeholder="Nº Mesa Ex: 04" value={pdvTableNum} onChange={(e) => setPdvTableNum(e.target.value)} className="bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
                <input type="text" placeholder="Nome Garçom" value={pdvWaiterName} onChange={(e) => setPdvWaiterName(e.target.value)} className="bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
              </div>
            )}
            ...
            <div className="space-y-2 relative">
              ...
              {pdvOrderType === 'delivery' && (
                <>
                  ...
                  <input type="text" placeholder="Endereço (Rua, Nº, Bairro)" value={pdvCustomer.address} onChange={(e) => setPdvCustomer({ ...pdvCustomer, address: e.target.value })} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
            ...
            <div className="space-y-2 border-t border-gray-800 pt-3">
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {pdvCart.length === 0 ? (
                  <p className="text-[11px] text-gray-500 italic py-2 text-center">Nenhum item adicionado.</p>
                ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 no-print">
          <section className="lg:col-span-1 bg-gray-900 p-5 rounded-2xl border border-gray-800 space-y-4 h-fit">
            ...
            <form onSubmit={handleAddProduct} className="space-y-3">
              <input type="text" placeholder="Nome do Produto" value={newProd.name} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewProd({ ...newProd, name: e.target.value })} />
              <input type="text" placeholder="Descrição curta" value={newProd.description} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewProd({ ...newProd, description: e.target.value })} />
              ...
              <div className="bg-gray-950 p-3.5 rounded-xl border border-purple-500/30 space-y-3">
                ...
                {newProd.is_combo ? (
                  <div className="space-y-2.5 pt-1">
                    {(newProd.combo_steps || []).map((step, idx) => (
                      <div key={idx} className="bg-gray-900 p-2.5 rounded-xl border border-gray-800 space-y-2">
                        ...
                        <input type="text" placeholder="Título da Etapa" value={step.title} onChange={(e) => updateComboStep(idx, 'title', e.target.value, 'new')} className="w-full bg-gray-800 border border-gray-700 p-2 rounded-lg text-xs text-white focus:outline-none" />
                  ...
                  <div className="space-y-2 pt-1 border-t border-gray-800">
                    <span className="text-xs font-bold text-orange-400 block">🍕 Opções de Pizza Comum</span>
                    <input type="number" min="0" placeholder="Limite Máximo de Sabores" value={newProd.max_addons} className="w-full bg-gray-800 border border-gray-700 p-2 rounded-lg text-xs text-white focus:outline-none font-bold" onChange={(e) => setNewProd({ ...newProd, max_addons: e.target.value })} />
                    <input type="text" placeholder="Bordas (Ex: Sem Borda:0, Catupiry:8.00)" value={newProd.borders_list} className="w-full bg-gray-800 border border-gray-700 p-2 rounded-lg text-xs text-white focus:outline-none" onChange={(e) => setNewProd({ ...newProd, borders_list: e.target.value })} />
                  </div>
                )}
              </div>

              <input type="text" placeholder="URL da Foto" value={newProd.image} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewProd({ ...newProd, image: e.target.value })} />
              ...
              {products.map((item) => (
                <div key={item.id} className="bg-gray-900 p-3.5 rounded-2xl border border-gray-800 flex justify-between items-center space-x-3">
                  ...
                  <div className="flex items-center space-x-1.5 shrink-0">
                    <button onClick={() => setEditingProduct(item)} className="text-xs bg-blue-600/20 text-blue-400 p-2 rounded-xl font-bold border border-blue-500/30">✏️</button>
                    ...
                    <button onClick={async () => { if (confirm("Excluir?")) { await supabase.from('products').delete().eq('id', item.id); fetchData(); } }} className="text-xs bg-red-500/20 text-red-400 p-2 rounded-xl font-bold">🗑</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 no-print">
          <section className="lg:col-span-1 bg-gray-900 p-5 rounded-2xl border border-gray-800 space-y-3 h-fit">
            ...
            <form onSubmit={handleAddGlobalAddon} className="space-y-3">
              ...
              <input type="text" placeholder="Descrição" value={newAddon.description} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewAddon({ ...newAddon, description: e.target.value })} />
              <input type="text" placeholder="Valor Adicional R$" value={newAddon.price} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" onChange={(e) => setNewAddon({ ...newAddon, price: e.target.value })} />
      ...
      {activeTab === 'clients' && (
        <div className="space-y-6 no-print">
          <section className="bg-gray-900 p-5 rounded-2xl border border-gray-800 space-y-1">
            ...
            <p className="text-xs text-gray-400">Histórico de compras e contatos via WhatsApp.</p>
          </section>
            ...
            {customerList.map((client, idx) => (
              <div key={idx} className="bg-gray-900 p-4 rounded-2xl border border-gray-800 space-y-3 flex flex-col justify-between">
                ...
                <div className="space-y-1.5">
                  <button onClick={() => setSelectedClientHistory(client)} className="w-full bg-blue-600/20 text-blue-400 border border-blue-500/30 py-2 rounded-xl text-xs font-bold">
                    📜 Ver Histórico de Pedidos
                  </button>
                  {client.phone && (
                    <button onClick={() => handleOpenPromoModal(client)} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 rounded-xl text-xs">
      ...
      {activeTab === 'reports' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-center bg-gray-900 p-4 rounded-2xl border border-gray-800 gap-3 no-print">
            ...
            <button onClick={() => window.print()} className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-4 py-2 rounded-xl text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800">
              ...
              <span className="text-2xl font-extrabold text-green-400">R$ {totalRevenue.toFixed(2)}</span>
            </div>
            <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800">
              <span className="text-xs text-gray-400 block mb-1">Total de Pedidos</span>
              <span className="text-2xl font-extrabold text-orange-400">{filteredOrders.length}</span>
            </div>
            <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800">
              <span className="text-xs text-gray-400 block mb-1">Subtotal Produtos</span>
              <span className="text-2xl font-extrabold text-blue-400">R$ {totalSubtotal.toFixed(2)}</span>
            </div>
            <div className="bg-gray-900 p-5 rounded-2xl border border-gray-800">
              <span className="text-xs text-gray-400 block mb-1">Taxas de Entrega</span>
              <span className="text-2xl font-extrabold text-purple-400">R$ {totalDeliveryFees.toFixed(2)}</span>
            </div>
          </div>
          ...
          <section className="bg-gray-900 p-5 rounded-2xl border border-red-500/30 flex justify-between items-center no-print">
            ...
            <button type="button" onClick={handleClearFinancialData} className="bg-red-500/20 hover:bg-red-500/40 text-red-400 border border-red-500/40 px-4 py-2.5 rounded-xl text-xs font-bold">
              🗑️ Limpar Pedidos
            </button>
          </section>
        </div>
        ...
        <div className="space-y-6">
          <section className="bg-gray-900 p-5 rounded-2xl border border-gray-800 space-y-3 no-print">
            ...
            <div className="flex items-center space-x-3 pt-1">
              <label className="text-xs font-bold text-gray-300">Qtd de Mesas:</label>
          ...
          <section className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {Array.from({ length: tableCount }, (_, i) => {
              ...
              return (
                <div key={i} className="bg-gray-900 p-4 rounded-2xl border border-gray-800 flex flex-col items-center space-y-2 text-center">
                  <span className="font-extrabold text-xs text-orange-400 truncate max-w-full">{tenant.name}</span>
                  <span className="font-bold text-xs bg-orange-500 text-white px-2.5 py-0.5 rounded-md">MESA {tableNum}</span>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 no-print">
          <div className="bg-gray-900 w-full max-w-sm rounded-2xl p-5 border border-blue-500/40 space-y-4">
            ...
            <div className="bg-gray-950 p-3 rounded-xl border border-gray-800 space-y-3">
              ...
              <div>
                <label className="text-[11px] text-gray-400 block mb-1">Dividir em quantas pessoas?</label>
                <input
                  type="number"
                  min="1"
                  max="50"
              ...
              <div className="bg-blue-600/10 border border-blue-500/30 p-3 rounded-xl text-center space-y-1">
                <span className="text-[10px] text-blue-300 block">Valor por pessoa:</span>
      ...
      {editingAddon && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 no-print">
          <form onSubmit={handleUpdateAddon} className="bg-gray-900 w-full max-w-sm rounded-2xl p-5 border border-blue-500/40 space-y-3">
            ...
            <select value={editingAddon.category_type || '🍕 Pizza Salgada'} onChange={(e) => setEditingAddon({ ...editingAddon, category_type: e.target.value })} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none font-bold">
              {ADDON_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
            ...
            <input type="text" placeholder="Descrição" value={editingAddon.description || ''} onChange={(e) => setEditingAddon({ ...editingAddon, description: e.target.value })} className="w-full bg-gray-800 border border-gray-700 p-2.5 rounded-xl text-xs text-white focus:outline-none" />
